...
import { getApplicationList, getVerifierApplicationList } from "@/server/application";
...
export default function AllVerifierApplicationComponent({ lockApplication }: Props) {
    ...
    const getVerifierApplicationDataList = useCallback(async () => {
        let response = await getVerifierApplicationList(page, 5, '');
    ...
    useEffect(() => {
        if (isInitialMount.current) {
            ...
            setHasMore(true);
            setTotalCount(0);
            getVerifierApplicationDataList();
    ...
    return (
        <InfiniteScroll
            ...
            next={page > 1 ? getVerifierApplicationDataList : () => { }}
            hasMore={hasMore}
            loader={<Loader />}
            endMessage={
